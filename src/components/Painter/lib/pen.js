    ...
    switch (view.type) {
      case 'text': {
        ...
        for (let i = 0; i < textArray.length; ++i) {
          ...
          // console.log('textArray textLength', textLength);
          const partWidth = view.css.width ? view.css.width.toPx() : textLength;
          // console.log('textArray partWidth', partWidth);
          let calLines = Math.ceil(textLength / partWidth);
          if (view.css.width) {
            let _lines = 1;
            let widthLength = 0;
            textArray[i].split('').map((text, j) => {
              const textWidth = this.ctx.measureText(text).width;
              widthLength += textWidth;

              if (partWidth - widthLength < view.css.fontSize.toPx() && j !== textArray[i].split('').length - 1) {
                widthLength = 0;
                _lines += 1;
              }
            });
        ...
        // console.log('linesArray lines', linesArray, lines);
  ...
  _fillAbsText(view) {
    ...
    if (view.id) {
      ...
      for (let i = 0; i < textArray.length; ++i) {
        ...
        // console.log('textWidth', textArray[i], textWidth);
      }
      this.globalWidth[view.id] = width ? (textWidth < width ? textWidth : width) : textWidth;
      // console.log('this.globalWidth[view.id]', this.globalWidth[view.id]);
    }
    let lineIndex = 0;
    for (let j = 0; j < textArray.length; ++j) {
      ...
      for (let i = 0; i < linesArray[j]; ++i) {
        ...
        // console.log('text measuredWith', text, measuredWith);
