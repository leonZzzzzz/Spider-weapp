...
import { Avatar, LogoWrap, LoadingBox } from '@/components/common'

export default function ActivitySign() {

  const [ pageLoading, setPageLoading ] = useState(true)
  const [ list, setList ] = useState([])
  const { id, signNum } = useRouter().params
  ...
  const signMembers = async () => {
    let res = await api.courseMemberBySign({ sourceId: id })
  ...
  return (
    <View className="sign">
      ...
      <View className="sign-title">已报名({signNum})</View>
